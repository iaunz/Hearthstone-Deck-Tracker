...
本仓库是 **Hearthstone Deck Tracker (HDT)** 的一个 fork（`v2` 分支）。HDT 是 Windows 上的炉石传说自动牌组追踪器/管理器（.NET Framework 4.7.2 / WPF / **x64**）：实时 overlay、手牌/牌库/抽牌概率追踪、对局统计、牌组管理，以及对**酒馆战棋（Battlegrounds）**的一整套支持（己方阵容、对手快照、英雄/饰品/任务选择、商店、BobsBuddy 战斗模拟等）。
...
"C:/Program Files (x86)/Microsoft Visual Studio/2022/BuildTools/MSBuild/Current/Bin/MSBuild.exe" <工程或sln> -p:Configuration=Debug -p:Platform=x64 -p:RuntimeIdentifier=win-x64 -v:minimal -nologo
```

```bash
# 跑测试（先构建，再加 --no-build —— 否则 dotnet 会用 SDK MSBuild 重建并触发 ResGen）
"C:/Program Files (x86)/Microsoft Visual Studio/2022/BuildTools/MSBuild/Current/Bin/MSBuild.exe" BgsDataBridge.Tests/BgsDataBridge.Tests.csproj -p:Configuration=Debug -p:Platform=x64 -p:RuntimeIdentifier=win-x64 -v:minimal -nologo
dotnet test BgsDataBridge.Tests/BgsDataBridge.Tests.csproj --no-build -p:Platform=x64 -p:RuntimeIdentifier=win-x64
```

要点：
- 目标框架 **net472**、平台 **x64**、语言 **C# 7.3**（上游 v1.53.x 已由 x86 迁移到 x64；HearthMirror/HearthDb 也随 Bootstrap 拉 x64 版本）。务必带 `-p:Platform=x64 -p:RuntimeIdentifier=win-x64`——主工程的 `<PlatformTarget>x64</PlatformTarget>` 会让 SDK 推导出 `win-x64` RID，NuGet 还原与构建的 RID 必须一致，否则报 `NETSDK1047`。
...
- **`Bootstrap/`（刷新 lib/）只在构建 `.sln` 时触发**——它是 solution 级 `ProjectDependency`，单独构建某个 `.csproj` 不会跑 Bootstrap，lib/ 也就不会更新。上游同步后若主工程报 `CS0117`（缺卡牌/GameTag）或 HearthMirror 报 `CS8012`（处理器不一致），说明 lib/ 过期：跑一次 `MSBuild Bootstrap/Bootstrap.csproj -p:Platform=x64`（或直接构建 `.sln`）重新拉取 HearthDb/HearthMirror 即可。
...
产物为 Costura 单 DLL：`BgsDataBridge/bin/x64/Release/BgsDataBridge.dll`。
